...
class ConversationValueUpdateResult(private val isUnarchived: Boolean, private val unreadIncrement: Int) {
	...
	fun emitUpdate(conversationInfo: ConversationInfo) {
		//Emitting updates
		if(isUnarchived) {
			ReduxEmitterNetwork.messageUpdateSubject.onNext(ConversationArchive(conversationInfo, false))
		}
		if(unreadIncrement > 0) {
			ReduxEmitterNetwork.messageUpdateSubject.onNext(ConversationUnread(conversationInfo, conversationInfo.unreadMessageCount + unreadIncrement))
		}
	}
}