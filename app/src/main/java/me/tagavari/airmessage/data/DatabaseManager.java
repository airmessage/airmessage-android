...
public class DatabaseManager extends SQLiteOpenHelper {
	...
	public List<ConversationInfo> fetchConversationsWithState(Context context, ConversationInfo.ConversationState conversationState) {
		...
		while(cursor.moveToNext()) {
			...
			LightConversationItem lightItem = getLastLightItem(context, chatID);
			...
			ConversationInfo conversationInfo = new ConversationInfo(chatID, chatGUID, conversationState, service, conversationMembers, chatTitle, chatUnreadMessages, chatColor, draftMessage, draftFiles, draftUpdateTime);
	...
	public MainApplication.LoadFlagArrayList<ConversationInfo> fetchSummaryConversations(Context context) {
		...
		while(cursor.moveToNext()) {
			...
			LightConversationItem lastItem = getLastLightItem(context, chatID);
			...
			//Creating and adding the conversation info
			ConversationInfo conversationInfo = new ConversationInfo(chatID, chatGUID, conversationState, service, conversationMembers, chatName, chatUnreadMessages, chatColor, draftMessage, draftFiles, draftUpdateTime);
			conversationInfo.setArchived(chatArchived);
			conversationInfo.setMuted(chatMuted);
	...
	private LightConversationItem getLastLightItem(Context context, long chatID) {
		//Getting the database
		SQLiteDatabase database = getReadableDatabase();
	...
	public ConversationInfo addRetrieveClientCreatedConversationInfo(Context context, List<String> members, String service) {
		//Getting the database
		SQLiteDatabase database = getWritableDatabase();
		
		//Returning an existing conversation if it exists
		ConversationInfo existingConversation = findConversationInfoWithMembers(context, members, service, false);
	...
	public ConversationInfo addRetrieveServerCreatedConversationInfo(Context context, String guid) {
		...
		//Returning the conversation info
		return new ConversationInfo(localID, guid, ConversationInfo.ConversationState.INCOMPLETE_SERVER);
	}
	...
	public ConversationInfo addRetrieveMixedConversationInfo(Context context, String guid, String[] members, String service) {
		...
		//Creating and configuring the conversation info
		ConversationInfo conversationInfo = new ConversationInfo(localID, guid, ConversationInfo.ConversationState.READY);
		conversationInfo.setConversationColor(conversationColor);
	...
	public ConversationInfo addReadyConversationInfo(Context context, Blocks.ConversationInfo structConversationInfo) {
		...
		//Creating and configuring the conversation info
		ConversationInfo conversationInfo = new ConversationInfo(localID, structConversationInfo.guid, ConversationInfo.ConversationState.READY);
	...
	public ConversationInfo findConversationInfoWithMembers(Context context, List<String> members, String service, boolean clientIncompleteOnly) {
		//Getting the database
		SQLiteDatabase database = getReadableDatabase();
		...
		Cursor conversationCursor = database.query(Contract.ConversationEntry.TABLE_NAME, new String[]{Contract.ConversationEntry._ID},
				...
				clientIncompleteOnly ? new String[]{service, Integer.toString(ConversationInfo.ConversationState.INCOMPLETE_CLIENT.getIdentifier())} : new String[]{service},
				null, null, null);
	...
	public ConversationInfo fetchConversationInfo(Context context, String conversationGUID) {
		...
		LightConversationItem lightItem = getLastLightItem(context, localID);
		...
		ConversationInfo conversationInfo = new ConversationInfo(localID, conversationGUID, conversationState, service, conversationMembers, chatTitle, chatUnreadMessages, chatColor, draftMessage, draftFiles, draftUpdateTime);
		conversationInfo.setArchived(chatArchived);
		conversationInfo.setMuted(chatMuted);
		conversationInfo.trySetLastItem(lightItem, false);
