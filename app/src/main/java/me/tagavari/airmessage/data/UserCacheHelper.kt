...
class UserCacheHelper {
	//Creating the values
	private val cache: LruCache<String, UserInfo>
	private val failedCache = Collections.synchronizedList(mutableListOf<String>())
	
	init {
		//Setting the user cache
		cache = object : LruCache<String, UserInfo>((Runtime.getRuntime().maxMemory() / 1024 / 8 / 2).toInt()) {
	 ...
	 */
	data class UserInfo constructor(val contactID: Long, val lookupKey: String, val contactName: String?) {
